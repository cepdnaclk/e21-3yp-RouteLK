...
class FirebaseBusService {
  ...
  Map<String, BusData> _parseBusEndpointResponse(
    ...
    for (final entry in rawBuses) {
      ...
      busData[busId] = BusData(
        ...
        direction: entry['direction'] is String ? (entry['direction'] as String).trim() : null,
      );
    }
  ...
  BusData? _parseBusLatest(Map<String, dynamic> value) {
    ...
    return BusData(
      ...
      direction: value['direction'] is String ? (value['direction'] as String).trim() : null,
    );
  }
  ...
  /// Set the active travel direction for a bus.
  Future<Map<String, dynamic>> setBusDirection({
    required String busId,
    required String direction,
  }) async {
    final mutation = '''
      mutation SetBusDirection {
        setBusDirection(input: {
          busId: ${jsonEncode(busId)}
          direction: ${jsonEncode(direction)}
        }) {
          busId
          direction
        }
    ...
    final data = await _postGraphql(mutation);
    final setDirection = data['setBusDirection'];
    if (setDirection is! Map<String, dynamic>) {
      throw Exception('AppSync setBusDirection mutation returned an unexpected payload');
    }

    return setDirection;
  }

  /// Get the active travel direction for a bus.
  Future<String?> getBusDirection(String busId) async {
    const query = r'''
      query GetBusDirection($busId: ID!) {
        getBusDirection(busId: $busId) {
          busId
          direction
        }
      }
    ...
    try {
      ...
      print('DEBUG: getBusDirection response for $busId: $data');

      final rawBus = data['getBusDirection'];
      if (rawBus is! Map<String, dynamic>) {
        print('DEBUG: getBusDirection rawBus is not a map or is null');
        return null;
      }

      final direction = rawBus['direction'] as String?;
      print('DEBUG: getBusDirection direction: $direction');
      return direction;
    } catch (e) {
      print('DEBUG: Error fetching bus direction for $busId: $e');
      developer.log('Error fetching bus direction: $e');
      return null;
    }
  }

  Future<void> cancelPickupRequest({
    required String pickId,
    required String busId,
    required String passengerId,
  }) async {
    const cancelMutation = '''
      mutation CancelBusPickup(\$input: CancelBusPickupInput!) {
        cancelBusPickup(input: \$input) {
          pickId
          busId
          passengerId
          status
    ...
    final cancelledPickup = data['cancelBusPickup'];
    if (cancelledPickup == null) {
      return;
    }
  }

  Future<void> arrivePickupRequest({
    required String pickId,
    required String busId,
    required String passengerId,
  }) async {
    const arriveMutation = '''
      mutation BusArrivedPickup(\$input: BusArrivedPickupInput!) {
        busArrivedPickup(input: \$input) {
          pickId
          busId
          passengerId
          status
    ...
    final arrivedPickup = data['busArrivedPickup'];
    if (arrivedPickup == null) {
      throw Exception('Failed to send arrival update to AWS');
    }
  }
  ...
  String _buildCancelBusPickupSubscription(String busId) {
    return '''
      subscription OnCancelBusPickup {
        onCancelBusPickup(busId: ${jsonEncode(busId)}) {
          pickId
          busId
          passengerId
          status
  ...
  Map<String, LatLng> _pickupMapAfterCancellation(
    Map<String, dynamic> pickup,
    Map<String, LatLng> currentMap,
  ) {
    final pickId = (pickup['pickId'] as String?)?.trim();
  ...
  Stream<Map<String, LatLng>> listenToPickupRequestsForBus(String busId) {
    ...
    void cancelRefreshTimer() {
      // No periodic refresh is used for cancel updates anymore.
    }
    ...
    Future<void> loadInitialPickupSnapshot() async {
      try {
    ...
    void startSubscription() {
      final createQuery = _buildCreateBusPickupSubscription(normalizedBusId);
      final cancelQuery = _buildCancelBusPickupSubscription(normalizedBusId);

      void sendStart(String id, String query) {
        channel?.sink.add(
      ...
      sendStart('${subscriptionId}-create', createQuery);
      sendStart('${subscriptionId}-cancel', cancelQuery);
    }
    ...
    Future<void> handleMessage(dynamic message) async {
      ...
      switch (type) {
        ...
        case 'data':
          ...
          final cancelled = data['onCancelBusPickup'];
          if (cancelled is! Map<String, dynamic>) {
            return;
          }

          final cancelledBusId = (cancelled['busId'] as String?)?.trim();
          if (cancelledBusId != normalizedBusId) {
            return;
          }

          final nextMap = _pickupMapAfterCancellation(cancelled, pickupMap);
          pickupMap
            ..clear()
            ..addAll(nextMap);
          if (!controller.isClosed) {
    ...
    Future<void> loadInitialPickups() async {
      await loadInitialPickupSnapshot();
