...
class _PassengerPickupStatusPageState extends State<PassengerPickupStatusPage> {
  ...
  Future<void> _arrivePickup() async {
    ...
    try {
      await _busService.arrivePickupRequest(
        pickId: widget.pickupId,
        busId: widget.busId,
        passengerId: widget.passengerId,
      );
      debugPrint('Successfully marked pickup request as arrived on AWS');
    } catch (e) {
      debugPrint('Error marking pickup request as arrived: $e');
    }
  ...
  void _listenToBusUpdates() {
    _busSubscription = _busService.listenToBusLocations().listen(
      (updatedBusMap) {
        ...
        // Check if the bus has arrived at the passenger pickup location (<= 50 meters)
        final distanceToPassenger = const Distance().as(
          LengthUnit.Meter,
          newLocation,
          widget.pickupLocation,
        );

        if (distanceToPassenger <= 50.0 && !_hasArrived) {
          _arrivePickup();
        }
                ...
                child: _hasArrived
                    ? Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Your bus ${widget.busId} has arrived at your location.',
                            textAlign: TextAlign.center,
                            style: TextStyle(
                              fontSize: 14,
