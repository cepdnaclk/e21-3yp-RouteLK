...
import 'route_direction_selection_page.dart';
...
class _DriverHomePageState extends State<DriverHomePage> {
  ...
  Future<void> _navigateToMap() async {
    ...
    try {
      // Fetch bus details from RDS
      final busDetails = await ApiService.getBusDetails(busId);
      final bus = busDetails['bus'] ?? {};
      ...
      if (route.isEmpty) {
        throw Exception('No registered route found for Bus $busId');
      }

      if (!mounted) return;

      // Navigate to direction selection page
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => RouteDirectionSelectionPage(
            busId: busId,
            route: route,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to load bus route: ${e.toString().replaceAll('Exception: ', '')}'),
